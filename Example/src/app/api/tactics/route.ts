...
export async function POST(request: NextRequest) {
  ...
  const agent =
    ...
    agentsList.find((a) => a.name === "Pitch") ??
    agentsList[0];

  const requiresHumanApproval = body.confidence < 70 || body.riskLevel === "high";
  ...
  if (requiresHumanApproval) {
    await db.insert(messages).values({
      ...
      parentId: tactic.id,
      authorType: "agent",
      authorId: agent?.id ?? "00000000-0000-0000-0000-000000000000",
